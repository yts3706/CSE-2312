...
_modulus1000:
   PUSH (LR)
   MOV R1, #1000	    @Setting it to 1000
   MOV R4, R0		    @Random value
   CMP R1, R4		    @Compare and see if R4 >= R1
   MOVHS R0, R4		    @Swap R1 and R4 if R4 >= R1
   MOVHS R4, R1		    @Swap R1 and R4 if R4 >= R1	
   MOVHS R1, R0
   MOV R0, #0
   B _loop
   _modulusloop
...
_printelements
   LDR R0, =print_maxelement
   MOV R1, R5
   BL printf
   LDR R0, =print_minelement
   MOV R1, R6
   BL printf
   B readdone
...
printf_minelement:     .asciz      "Min Value = %d\n"
printf_maxelement:     .asciz      "Max Value = %d\n"
debug_str:
